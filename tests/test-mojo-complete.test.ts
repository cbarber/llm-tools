...
beforeAll(async () => {
  ...
  // Start opencode in a minimal fixture dir; each suite creates its own session dir
...
// ---------------------------------------------------------------------------
// Suite 3: reset on git commit lets mojo-complete fire again
// ---------------------------------------------------------------------------
...
describe("mojo-complete — reset on git commit allows re-fire", () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      process.stderr.write("\n=== TEMPER EVENT LOG (suite 3) ===\n" + log + "\n=== END TEMPER EVENT LOG ===\n");
    } catch { /* no log */ }
    await rm(dir, { recursive: true, force: true }).catch(() => { });
  }, 90_000);
  ...
  it("fires twice total: once before commit, once after reset", () => {
    ...
    if (appearances.length !== 2) {
      logDiagnostics(mock, `mojo-complete appeared ${appearances.length} time(s), expected 2`);
    }
