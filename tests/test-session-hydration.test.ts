...
describe("temper plugin — firedOnce hydration on restart (synthetic injection)", () => {
  ...
  beforeAll(async () => {
    ...
    let { restart, ...rest } = await startOpencode(dir, ocPort);
    stopOpencode = rest.stop;
  ...
  it("mojo-init appears exactly once before restart", () => {
    ...
    let found = 0;
    for (const m of tasks[requestCountBeforeRestart - 1].request.messages) {
  ...
  it("mojo-init does NOT re-appear in requests after restart — blocked by OpenCode lazy session history loading", () => {
    const tasks = taskRequests(mock);
    expect(tasks.length).toBeGreaterThan(requestCountBeforeRestart);
    let found = 0;
...
describe("temper plugin — firedOnce hydration on restart (skill tool call)", () => {
  ...
  it("mojo-init does NOT appear after restart when skill was tool-called before — blocked by OpenCode lazy session history loading", () => {
    const tasks = taskRequests(mock);
    expect(tasks.length).toBeGreaterThan(requestCountBeforeRestart);
    let found = 0;
    for (const m of tasks[tasks.length - 1].request.messages) {
