...
export const TemperPlugin: Plugin = async ({ client, $, directory, serverUrl }) => {
  ...
  async function hydrateSession(sessionID: string, skills: Skill[]): Promise<SessionState> {
    ...
    try {
      ...
      for (const { parts } of messages) {
        for (const part of parts) {
          if (part.type === "text" && part.synthetic) {
            ...
            if (name && onceSkillNames.has(name)) {
      ...
      await logEvent(logPath, "dispatch-hydrate", { sessionID, foundInHistory });
    } catch (error) {
  ...
  async function dispatchEvent(
    ...
    if (sessionStore.has(sessionID)) {
      ...
      if (!pendingHydration.has(sessionID)) {
        pendingHydration.set(sessionID, hydrateSession(sessionID, skills).finally(() => pendingHydration.delete(sessionID)));
      }
